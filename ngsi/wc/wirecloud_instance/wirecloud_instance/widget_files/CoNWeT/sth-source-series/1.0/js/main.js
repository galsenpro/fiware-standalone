...
(function () {
    ...
    var requestData = function requestData(entity) {

        if (entity && !(entity === mp.prefs.get('entity'))) {
          var ent = JSON.parse(entity).id;
          entity = ent;
          // console.log('changed to entity :' + entity);
        } else {
          entity = mp.prefs.get('entity');
        }
        ...
        mp.http.makeRequest(url, {
            ...
            onSuccess: function (response) {
                ...
                var seri1 =  data.map(function (entry) {
                       var elem = [(new Date(entry.recvTime)).getTime(), Number(entry.attrValue)];
                       return elem;
                   });
                var serie = {
                     series:[
                       {
                         name: mp.prefs.get('attribute'),
                         data: seri1
                       }
                     ]
                };

                // console.log('serie: %o', serie);
                ...
                mp.wiring.pushEvent("timeserie", serie);
            },
